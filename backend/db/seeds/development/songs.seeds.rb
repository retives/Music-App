...
after 'development:albums', 'development:artists', 'development:genres', 'development:playlists' do
  artists = Artist.all
  genres = Genre.all
  ...
  cyrillic_songs.each do |song|
    Song.create(
      title: song['title'],
      album: Album.find_or_create_by!(title: song['album']),
      genre: genres.sample,
      created_at: 1.year.ago,
      updated_at: 1.year.ago
    )
  ...
  Song.find_each do |song|
    ...
    rand(1..playlists.count).times do
      ...
      PlaylistSong.create(
        ...
        song:,
        user_id: selected_playlist_data[:user_id]
      )
    end
  end
end
